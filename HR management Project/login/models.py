...
class Login(models.Model):
	...
	image=models.ImageField(upload_to='static', height_field=None, width_field=None,max_length=100, null=True, blank=True)
...
class Daily(models.Model):
	...
	def create_dailyreport(self,request,title,report,myfile,workhour):
		...
		e=Daily.objects.create(title=title,dailyreport=report,uploadfile=myfile,datetime=d,workhours=workhour,User=l)
...
class Notification(models.Model):
    ...
    def get_notification(self,request):
         n=Notification.objects.filter(seen=False)
         n1=n.values()
         return n1
